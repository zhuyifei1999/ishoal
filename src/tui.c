...
static void tui_on_ikiwi_addr_change(int fd, void *_ctx, bool expired)
{
	recompute_title();
	refresh();
}
...
static struct event tui_global_events[] = {
	...
	},
	{
		.eventfd_ack = true,
		.handler_type = EVT_CALL_FN,
		.handler_fn = tui_on_ikiwi_addr_change,
	},
};
...
void tui_thread(void *arg)
{
	...
	tui_global_events[1].fd = broadcast_replica(ikiwi_addr_set_broadcast);
